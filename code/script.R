...
## Load data ----
  ...
  # nestling encounter data: one row per encounter (some nestlings have multiple encounters)
    d_enc <- read.delim(here::here("data", "encounters.txt"))
    
  # weather data from ithaca airport (one row per hour)
    # provided by Northeast regional climate center
    d_weather <- read.delim(here::here("data", "ithaca_airport_weather.txt"))
...
## Filter data out to start initial visualization 
initial_vis <- d_enc %>%
  select(sex, individual_treatment, nest_treatment,age, stress_series_type, bleed1_final_cort, avgC_capture_day) %>%
  filter(
    !is.na(sex),
...
write.csv(initial_vis, "9-7-25_data.csv", row.names = FALSE)