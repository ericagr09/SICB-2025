...
TLDR: should likely include encounter day of year and body mass as covariates. filtering down to 120 for latency does seem to weaken the relationship with overall cort, but also removes ~130 observations. maybe should include year as a covariate as well, since most cold days will likely be from the same year 
...
vis0 <- ggplot(df, aes(x = bleed1_latency_sec, y = bleed1_final_cort)) +
  geom_point(alpha = 0.5) +     
  geom_smooth(method = "lm", se = FALSE)
vis0

```
Now, going to sample to under 120 to see if that weakens the relationship 

```{r plot0.1}
df1 <- df %>%
    filter(bleed1_latency_sec <= 120)

vis01 <- ggplot(df1, aes(x = bleed1_latency_sec, y = bleed1_final_cort)) +
  geom_point(alpha = 0.5) +     
  geom_smooth(method = "lm", se = FALSE)
vis01
```
