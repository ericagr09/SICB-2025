...
Hypothesis 3:Based on the differing literature on developmental corticosterone in birds, we do not have enough basis to predict a relationship or differential impacts of temperature on corticosterone.
...
```{r}
bleed1_int <- lmer(bleed1_final_cort ~ avgC_capture_day + sex + avgC_capture_day*sex + bleed1_latency_sec + (1 | nest_key), data = sex_df)
summary(bleed1_int)
```
...
```{r}
bleed2_int <- lmer(bleed2_final_cort ~ avgC_capture_day + sex + avgC_capture_day*sex + bleed1_final_cort + (1 | nest_key), data = sex_df)
summary(bleed2_int)
```
...
```{r}
bleed3_int <- lmer(bleed3_final_cort ~ avgC_capture_day + sex + avgC_capture_day*sex + bleed1_final_cort + (1 | nest_key), data = sex_df)
summary(bleed3_int)
```
